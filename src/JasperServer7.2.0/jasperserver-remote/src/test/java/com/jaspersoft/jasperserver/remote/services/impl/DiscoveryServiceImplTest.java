...
public class DiscoveryServiceImplTest {
    ...
    public void discover_externalURL_existManyManyCorrectReverse() throws Exception {
        when(repositoryService.getResource(any(ExecutionContext.class), eq(l2.getURIString()))).thenReturn(resource);
        when(repositoryService.findResource(any(ExecutionContext.class), any(FilterCriteria.class))).thenReturn(new ResourceLookup[]{l1, l5, l2, l4});
        when(provider.getDiscoveryStrategyFor(any(Resource.class))).thenReturn(strategy);
